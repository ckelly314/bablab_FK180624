...
if method == 'redfield':
    ...
    DNRN_NO3 = round(2*C+0.5*H-O-1.5*N+2.5*P,2) # This comes from balancing the oxygen atoms in the DNRN reaction in Table 1 and solving for "x"
    DNRN_NO2 = DNRN_NO3
    DNRN_water = round(0.5*H-1.5*N-1.5*P,2) # This comes from balancing the hydrogen atoms in the DNRN reaction and solving for "y"
    denit_NO2 = round(4/3*C+1/3*H-2/3*O-N+5/3*P,2) # in case of confusion, "denit" here refers specifically to NO2- reduction to N2O
